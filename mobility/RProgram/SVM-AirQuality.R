...
# Author: iotcloud Benedict Date: 19.2.2021
# SVM -- Algorithm
#
##############################################################################################
...
############################ Model data
################### SVM regression is used to avoid negative values while predicting the energy
library(e1071)
stime_SO2 <- proc.time()
#model_SO2 <- svm(NumericSO2 ~ SeqNoSO2 + NumericNO2 + LatLong, data = trainingSO2, kernel="linear", scale=FALSE, type ="eps-regression", cost = 1, nu = 0.5, cachesize = 100, tolerance = 0.1, epsilon = 0.1, probability=TRUE)
model_SO2 <- svm(NumericSO2 ~ SeqNoSO2 + NumericNO2 + LatLong, data = trainingSO2, kernel="radial", probability=TRUE, family=poisson)
etime_SO2 <- proc.time() - stime_SO2
print(etime_SO2)

stime_NO2 <- proc.time()
#model_NO2 <- svm(NumericNO2 ~ SeqNoNO2 + NumericSO2 + LatLong, data = trainingNO2, kernel="linear", scale=FALSE, type = NULL, degree = 3, coef0 = 0, cost = 1, nu = 0.5, cachesize = 40, tolerance = 0.001, epsilon = 0.1, probability=TRUE)
model_NO2 <- svm(NumericNO2 ~ SeqNoNO2 + NumericSO2 + LatLong, data = trainingNO2,kernel="radial", probability=TRUE, family=poisson)
...
predicted_SO2 <- predict(model_SO2, testingSO2, type="response")
epredicttimeSO2 <- proc.time() - spredicttimeSO2
print(epredicttimeSO2)
spredicttimeNO2 <- proc.time()
predicted_NO2 <- predict(model_NO2, testingNO2, type="response")
...
if(predict_busroute_airquality == "FALSE"){
	...
	### Print pdf for NO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/SVM_NO2Plot.pdf",width=8,height=5)
	print(plotNO2)
	dev.off()
	...
	### Print pdf for SO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/SVM_SO2Plot.pdf",width=8,height=5)
	print(plotSO2)
	dev.off()
