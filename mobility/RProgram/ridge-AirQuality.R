...
# Ridge Regression - L2 Regularization to deal with multi-collinearity problem -- Algorithm
#
##############################################################################################
...
############################ Model data
################### ridge regression - glmnet is used to avoid negative values while predicting the energy
library(tidyverse)
library(broom)
library(glmnet)
ind_var_SO2 <- trainingSO2$NumericSO2
dep_var_SO2 <- trainingSO2 %>% select(SeqNoSO2, NumericNO2, LatLong) %>% data.matrix()
lambdasSO2 <- 10^seq(3, -2, by = -.1)
ind_var_NO2 <- trainingNO2$NumericNO2
dep_var_NO2 <- trainingNO2 %>% select(SeqNoNO2, NumericSO2, LatLong) %>% data.matrix()
lambdasNO2 <- 10^seq(3, -2, by = -.1)

stime_SO2 <- proc.time()
model_SO2 <- cv.glmnet(dep_var_SO2, ind_var_SO2, alpha=0, lambda = lambdasSO2)
etime_SO2 <- proc.time() - stime_SO2
print(etime_SO2)

stime_NO2 <- proc.time()
model_NO2 <- cv.glmnet(dep_var_NO2, ind_var_NO2, alpha=0, lambda = lambdasNO2)
etime_NO2 <- proc.time() - stime_NO2
print(etime_NO2)

optimal_lambdaSO2 <- model_SO2$lambda.min
optimal_lambdaNO2 <- model_NO2$lambda.min

#predict the outcome of the testing data 
spredicttimeSO2 <- proc.time()
predicted_SO2 <- predict(model_SO2, s = optimal_lambdaSO2, newx = dep_var_SO2)
epredicttimeSO2 <- proc.time() - spredicttimeSO2
print(epredicttimeSO2)
spredicttimeNO2 <- proc.time()
predicted_NO2 <- predict(model_NO2, s = optimal_lambdaNO2, newx = dep_var_NO2)
...
if(predict_busroute_airquality == "FALSE"){
	...
	### Print pdf for NO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/Ridge_NO2Plot.pdf",width=8,height=5)
	print(plotNO2)
	dev.off()
	...
	### Print pdf for SO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/Ridge_SO2Plot.pdf",width=8,height=5)
	print(plotSO2)
	dev.off()
