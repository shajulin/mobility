...
# ElasticNet Regression -- A combination of both lasso and ridge regressions
# 
##############################################################################################
...
############################ Model data
################### Elastic Net Regression - caret package 
library(caret)
...
stime_SO2 <- proc.time()
model_SO2 <- train(NumericSO2 ~ SeqNoSO2 + NumericNO2 + LatLong, data = trainingSO2, method="glmnet", trControl=train_cont)
etime_SO2 <- proc.time() - stime_SO2
print(etime_SO2)

stime_NO2 <- proc.time()
model_NO2 <- train(NumericNO2 ~ SeqNoNO2 + NumericSO2 + LatLong, data = trainingNO2, method="glmnet", trControl=train_cont)
...
predicted_SO2 <- predict(model_SO2, testingSO2, type="raw", predict.all=FALSE)
epredicttimeSO2 <- proc.time() - spredicttimeSO2
print(epredicttimeSO2)
spredicttimeNO2 <- proc.time()
predicted_NO2 <- predict(model_NO2, testingNO2, type="raw", predict.all=FALSE)
...
if(predict_busroute_airquality == "FALSE"){
	...
	### Print pdf for NO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/EN_NO2Plot.pdf",width=8,height=5)
	print(plotNO2)
	dev.off()
	...
	### Print pdf for SO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/EN_SO2Plot.pdf",width=8,height=5)
	print(plotSO2)
	dev.off()
