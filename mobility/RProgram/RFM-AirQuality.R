...
############################ Model data
################### RFM - Random Forest - glm is used to avoid negative values while predicting the energy
if(predict_busroute_airquality == "FALSE"){
	library(randomForest)
	stime_SO2 <- proc.time()
	model_SO2 <- randomForest(NumericSO2 ~ SeqNoSO2 + NumericNO2 + NumericLocation + Latitude + Longitude, data = trainingSO2, importance=TRUE, keep.forest=TRUE, ntree=100, mtry=2)
