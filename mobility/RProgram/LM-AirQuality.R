...
############################ Model data
################### Linear Regression -- Normal one-- is used to avoid negative values while predicting the energy
stime_SO2 <- proc.time()
model_SO2 <- lm(NumericSO2 ~ SeqNoSO2 + NumericNO2 + LatLong, data = trainingSO2)
etime_SO2 <- proc.time() - stime_SO2
print(etime_SO2)

stime_NO2 <- proc.time()
model_NO2 <- lm(NumericNO2 ~ SeqNoNO2 + NumericSO2 + LatLong, data = trainingNO2)
...
if(predict_busroute_airquality == "FALSE"){
	...
	### Print pdf for NO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/LM_NO2Plot.pdf",width=8,height=5)
	print(plotNO2)
	dev.off()
	...
	### Print pdf for SO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/LM_SO2Plot.pdf",width=8,height=5)
	print(plotSO2)
	dev.off()
