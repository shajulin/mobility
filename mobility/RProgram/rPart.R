...
maxNumNO2 <- nrow(OrderDataNO2)
maxNumSO2 <- nrow(OrderDataSO2)

OrderDataNO2$SeqNoNO2 <- seq(1,maxNumNO2)
OrderDataSO2$SeqNoSO2 <- seq(1,maxNumSO2)
...
if(predict_busroute_airquality == "FALSE"){
	library(rpart)
	stime_SO2 <- proc.time()
	model_SO2 <- rpart(NumericSO2 ~ SeqNoSO2 + NumericNO2 + NumericLocation + Latitude + Longitude, data = trainingSO2)
	etime_SO2 <- proc.time() - stime_SO2
	print(etime_SO2)

	stime_NO2 <- proc.time()
	model_NO2 <- rpart(NumericNO2 ~ SeqNoNO2 + NumericSO2 + NumericLocation + Latitude + Longitude + NumericSO2, data = trainingNO2)
...
predicted_SO2 <- predict(model_SO2, testingSO2, type="vector")
epredicttimeSO2 <- proc.time() - spredicttimeSO2
print(epredicttimeSO2)
spredicttimeNO2 <- proc.time()
predicted_NO2 <- predict(model_NO2, testingNO2, type="vector")
...
if(predict_busroute_airquality == "FALSE"){
	...
	### Print pdf for NO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/rpart_NO2Plot.pdf",width=8,height=5)
	print(plotNO2)
	dev.off()
	...
	### Print pdf for SO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/rpart_SO2Plot.pdf",width=8,height=5)
	print(plotSO2)
	dev.off()
