...
# copyright IoT Cloud Research Lab -- Deep Neural Network (ANN approach)
# Author: iotcloud Benedict Date: 1.6.2020
# RFM -- Algorithm
# http://uc-r.github.io/ann_regression (Based on )
##############################################################################################
...
predict_busroute_airquality <- "FALSE"
...
#Filtered data
trainingSO2Filtered <- trainingSO2[c("Year", "Stn.Code", "Latitude", "Longitude", "LatLong", "NumericNO2", "NumericSO2", "DateLocation", "SeqNoSO2")] 
...
# Mutate training and testing 
#trainingSO2Filtered_mutate <- trainingSO2Filtered %>%
#  mutate_all(scale01)
#trainingSO2 <- trainingSO2Filtered_mutate

trainingSO2 <- trainingSO2Filtered 
...
if(predict_busroute_airquality == "FALSE"){
	...
	model_SO2 <- neuralnet(NumericSO2 ~ SeqNoSO2 + NumericNO2 + Latitude + Longitude, data = trainingSO2, hidden=8, linear.output=TRUE)
		
	predicted_SO2 <- predict(model_SO2, testingSO2, type="vector", predict.all=FALSE)
	print(predicted_SO2)
	...
	stime_NO2 <- proc.time()
	model_NO2 <- randomForest(NumericNO2 ~ SeqNoSO2 + NumericNO2 + NumericLocation + Latitude + Longitude + SeqNoNO2 + NumericSO2, data = trainingNO2, importance=TRUE, keep.forest=TRUE, ntree=100, mtry=2)
	etime_NO2 <- proc.time() - stime_NO2
	print(etime_NO2)
} else {
	library(randomForest)
	stime_SO2 <- proc.time()
	model_SO2 <- randomForest(NumericSO2 ~ NumericLocation + Latitude + Longitude, data = trainingSO2, importance=TRUE, keep.forest=TRUE, ntree=100, mtry=2)
	etime_SO2 <- proc.time() - stime_SO2
	print(etime_SO2)

	stime_NO2 <- proc.time()
	model_NO2 <- randomForest(NumericNO2 ~ NumericLocation + Latitude + Longitude, data = trainingNO2, importance=TRUE, keep.forest=TRUE, ntree=100, mtry=2)
	etime_NO2 <- proc.time() - stime_NO2
	print(etime_NO2)
}
...
predicted_SO2 <- predict(model_SO2, testingSO2, type="response", predict.all=FALSE, proximity=FALSE, nodes=FALSE)
epredicttimeSO2 <- proc.time() - spredicttimeSO2
print(epredicttimeSO2)
spredicttimeNO2 <- proc.time()
predicted_NO2 <- predict(model_NO2, testingNO2, type="response", predict.all=FALSE, proximity=FALSE, nodes=FALSE)
...
if(predict_busroute_airquality == "FALSE"){
	...
	### Print pdf for NO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/RFM_NO2Plot.pdf",width=8,height=5)
	print(plotNO2)
	dev.off()
	...
	### Print pdf for SO2
	pdf("/home/iotcloud/shaju/workspace/Eclipseworkspace/EnergyAnalyzer/tests/mobility/Results/RFM_SO2Plot.pdf",width=8,height=5)
	print(plotSO2)
	dev.off()
