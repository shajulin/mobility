...
############################ Model data
################### Quantile Regression - quantreg is used to avoid negative values while predicting the energy
library(quantreg)
stime_SO2 <- proc.time()
model_SO2 <- rq(NumericSO2 ~ SeqNoSO2 + NumericNO2 + LatLong, data = trainingSO2, tau=0.25)
etime_SO2 <- proc.time() - stime_SO2
print(etime_SO2)

stime_NO2 <- proc.time()
model_NO2 <- rq(NumericNO2 ~ SeqNoNO2 + NumericSO2 + LatLong, data = trainingNO2, tau=0.25)
